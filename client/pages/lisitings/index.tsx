...
const Listing = ({products, page_size, cartData}) => {
     ...
     const addProduct= async()=>{
          await fetch(`http://${process.env.NEXT_PUBLIC_HOST}:8000/costumer/cart/`,{
               method:"POST",
               headers: {
...
export async function getServerSideProps (context){
     ...
     const res = await fetch(`http://${process.env.NEXT_PUBLIC_HOST}:8000/costumer/products/?page_size=${page_size}`,{
       headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
  ...
  const res2 = await fetch(`http://${process.env.NEXT_PUBLIC_HOST}:8000/costumer/cart/`,{
   ...
   if (category){
     const res = await fetch(`http://${process.env.NEXT_PUBLIC_HOST}:8000/costumer/products/?page_size=${page_size}&category=${category}`,{
  headers: {
       'Accept': 'application/json',
       'Content-Type': 'application/json'
...
return{props:{ products, page_size, cartData}}
   ...
   if (lowprice && highprice){
     const res = await fetch(`http://${process.env.NEXT_PUBLIC_HOST}:8000/costumer/products/?page_size=${page_size}&lowprice=${lowprice}&highprice=${highprice}`,{
  headers: {
       'Accept': 'application/json',
       'Content-Type': 'application/json'
...
if (search){
     const res = await fetch(`http://${process.env.NEXT_PUBLIC_HOST}:8000/costumer/products/?page_size=${page_size}&search=${search}`,{
  headers: {
       'Accept': 'application/json',
       'Content-Type': 'application/json'
